...
#define faceWidth 20.0
#define faceLineWidth 0.5
#define faceEyeWith 3.0
#define CSDNLogoImageTag 2222
...
@interface CSDNQuanRefreshImage : UIView

@property (nonatomic, strong) UIColor *arrowColor;
@property (nonatomic, assign) BOOL closeLeftEye;
@property (nonatomic, assign) BOOL closeRightEye;
@property (nonatomic, assign) BOOL closeMouth;
@property (nonatomic, strong) NSTimer *updateEyeTimer;
-(void)startUpdateEye;
-(void)stopUpdateEye;
- (void)updateFaceEye:(NSTimer *)theTimer;
@end
...
@property (nonatomic, strong) UIActivityIndicatorView *arrow;
...
@property(nonatomic,strong)UIImageView * CSDNLogo;
...
-(void)showNoNataView:(NSString*)alerStr imageName:(NSString*)name
{
    ...
    if([self viewWithTag:NodataViewTag] == nil)
    {
        ...
        UIView * noDataView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height/2-nodataHeight/2, mScreenWidth, nodataHeight)];
        
        UIImageView * nodatalogo = [[UIImageView alloc] initWithFrame:CGRectMake(mScreenWidth/2-15, 0, 30, 30)];
        nodatalogo.tag = NoDataLogoTag;
        [noDataView addSubview:nodatalogo];
        UILabel * nodatalabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 50, mScreenWidth, 10) ];
        
        nodatalabel.textAlignment = NSTextAlignmentCenter;
        nodatalabel.textColor= mRGBToColor(0xbfbfbf);
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.CSDNLogo = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"csdnGray"]];
        self.CSDNLogo.tag = CSDNLogoImageTag;
        [self addSubview:self.CSDNLogo];
        [self sendSubviewToBack:self.CSDNLogo];
    }
...
- (void)layoutSubviews {
    ...
    self.CSDNLogo.center = CGPointMake(self.scrollView.frame.size.width/2.0
                                      , SVPullToRefreshViewHeight+self.CSDNLogo.frame.size.height/2.0+10);
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            ...
            case SVPullToRefreshStateStopped:
                ...
                else if([self.arrow isKindOfClass:[UIActivityIndicatorView class]])
                {
                    [self.activityIndicatorView stopAnimating];
                ...
                switch (self.position) {
                    case SVPullToRefreshPositionTop:
                        [self viewWithTag:CSDNLogoImageTag].hidden = YES;
                        if([self.arrow isKindOfClass:[SVPullToRefreshArrow class]])
                    ...
                    case SVPullToRefreshPositionBottom:
                        ...
                        if([self.arrow isKindOfClass:[UIActivityIndicatorView class]])
                            {
                ...
                switch (self.position) {
                    case SVPullToRefreshPositionTop:
                        if(self.scrollView.contentSize.height>0)
                            [self viewWithTag:CSDNLogoImageTag].hidden = NO;
                       
                        [self.scrollView hideNodataView];
                        if([self.arrow isKindOfClass:[SVPullToRefreshArrow class]])
                        [self rotateArrow:(float)M_PI hide:NO];
                        else
                            if([self.arrow isKindOfClass:[UIActivityIndicatorView class]])
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                yOrigin = self.frame.origin.y>-60?-SVPullToRefreshViewHeight:self.frame.origin.y;
                break;
            case SVPullToRefreshPositionBottom:
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    //UALog(@"scroll的移动%.1f 上面的位置： %.1f",contentOffset.y,self.frame.origin.y);
    //下拉控制是否移动头部
...
- (UIActivityIndicatorView *)arrow {
    if(!_arrow) {
		_arrow = [[UIActivityIndicatorView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, faceWidth, faceWidth)];
        _arrow.backgroundColor = [UIColor clearColor];
        _arrow.color = [UIColor grayColor];
...
#pragma mark - CSDNQuanRefreshImage

@implementation CSDNQuanRefreshImage
@synthesize arrowColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect bodyRect= CGRectMake(faceLineWidth, faceLineWidth, faceWidth-2*faceLineWidth, faceWidth-2*faceLineWidth);
    CGRect lefteyeRect= CGRectMake(faceWidth/2.0-faceWidth/6.0-faceEyeWith/2.0, faceWidth/2.0-faceWidth/6.0, faceEyeWith, faceEyeWith);
    CGRect righteyeRect= CGRectMake(faceWidth/2.0+faceWidth/6.0-faceEyeWith/2.0, faceWidth/2.0-faceWidth/6.0, faceEyeWith, faceEyeWith);
    CGContextSetStrokeColorWithColor(context, self.arrowColor.CGColor);
    CGContextSetLineWidth(context, faceLineWidth);
    ...
    //
    CGContextMoveToPoint(context, faceWidth/2.0+faceWidth*(sinf(M_PI/4.0))/4.0, faceWidth/2.0+faceWidth*(sinf(M_PI/4.0))/4.0);
    CGContextAddArc(context,faceWidth/2.0,faceWidth/2.0,faceWidth/4.0,M_PI/4.0,M_PI*3.0/4.0,0);
    if(self.closeMouth)
    {
...
- (void)startUpdateEye
{
    if (self.updateEyeTimer)
        return;
    self.updateEyeTimer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self
                                                          selector:@selector(updateFaceEye:) userInfo:nil repeats:YES];
...
- (void)stopUpdateEye
{
    if (self.updateEyeTimer)
    {
        [self.updateEyeTimer invalidate];
        self.updateEyeTimer = nil;
    }
...
- (void)updateFaceEye:(NSTimer *)theTimer
{
    ...
    if(self.tag%2==0)
    {
        self.closeLeftEye = YES;
        self.closeRightEye = NO;
        self.closeMouth = NO;
        self.tag++;
    }
    else
    {
        self.closeLeftEye = NO;
        self.closeRightEye = YES;
        self.closeMouth = NO;
        self.tag--;
